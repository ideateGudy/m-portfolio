"use client";

import React from "react";

import { companies, testimonials } from "@/data";
import { InfiniteMovingCards } from "./ui/infinite-moving-cards";
import { SectionReveal, StaggerItem } from "./ui/section-reveal";
import { motion } from "motion/react";

const Clients = () => {
  return (
    <section id="testimonials" className="py-20 sm:py-24 w-full relative">
      <SectionReveal stagger staggerDelay={0.08}>
        <StaggerItem direction="up" offset={16} className="flex flex-col items-center justify-center text-center">
          <span className="uppercase tracking-widest text-xs font-semibold px-4 py-1.5 rounded-full border border-purple/30 bg-purple/10 text-purple mb-4">
            Client Feedback
          </span>
          <h2 className="heading max-w-2xl">
            Real results from <span className="text-purple">real collaborations</span>
          </h2>
          <p className="text-white-200 mt-4 max-w-xl text-sm md:text-base font-light">
            Feedback from startup founders, product managers, and engineering leaders who trusted me with their mission-critical architecture.
          </p>
        </StaggerItem>

        <div className="flex flex-col items-center mt-12 w-full">
          <StaggerItem direction="up" offset={16} className="w-full flex flex-col antialiased items-center justify-center relative overflow-hidden py-4">
            <InfiniteMovingCards
              items={testimonials}
              direction="right"
              speed="slow"
            />
          </StaggerItem>

          <StaggerItem direction="up" offset={16} className="flex flex-wrap items-center justify-center gap-4 md:gap-16 max-lg:mt-10">
            {companies.map((company) => (
              <motion.div
                key={company.id}
                whileHover={{ scale: 1.06, y: -2, transition: { duration: 0.18, ease: "easeOut" } }}
                className="flex md:max-w-60 max-w-32 gap-2 cursor-pointer transition-opacity duration-200 hover:opacity-100 opacity-80"
              >
                <img
                  src={company.img}
                  alt={company.name}
                  loading="lazy"
                  decoding="async"
                  width={40}
                  height={40}
                  className="md:w-10 w-5 object-contain"
                />
                <img
                  src={company.nameImg}
                  alt={company.name}
                  loading="lazy"
                  decoding="async"
                  width={company.id === 4 || company.id === 5 ? 100 : 150}
                  height={40}
                  className="md:w-24 w-20 object-contain"
                />
              </motion.div>
            ))}
          </StaggerItem>
        </div>
      </SectionReveal>
    </section>
  );
};

export default Clients;